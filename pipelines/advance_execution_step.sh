...
# Ce script fait avancer un étape spécifique d'exécution d'un pipeline dans Adobe Cloud Manager.
# Le script utilise une requête PUT pour déclencher l'avancement de l'étape d'exécution en cours pour un pipeline donné.
...
# Préparation de la commande curl pour avancer l'étape d'exécution
curl_command="curl -s -w '%{http_code}' -X PUT \"https://${host_name}/api/program/$program_id/pipeline/$pipeline_id/execution/$execution_id/advance\" \
