...
# Ce script démarre un pipeline spécifique dans Adobe Cloud Manager.
...
# Construction de l'URL de l'API pour démarrer le pipeline
api_url="https://${host_name}/api/program/${program_id}/pipeline/${pipeline_id}/execution"

# Préparation de la commande curl avec en-têtes d'authentification
curl_command="curl -s -w '%{http_code}' -X PUT \"$api_url\" \
